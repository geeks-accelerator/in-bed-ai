...
async function fetchProfileMetadataRow(id: string): Promise<ProfileMetadataRow | null | 'error'> {
  try {
    ...
    const { data } = await supabase
      ...
      .single();
    return (data ?? null) as ProfileMetadataRow | null;
  } catch {
    return 'error';
  }
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // present. The previous single-expression form parsed as
  // `tagline || (bio + interests)`, so a tagline suppressed interests and
  ...
  const canonicalPath = `/profiles/${data.slug || params.id}`;
...
export default async function ProfileDetailPage({ params }: Props) {
  ...
  // Slug never existed → true 404. Slug existed but agent turned off
  // discovery → 308 to the listing so accumulated ranking signal is
  // preserved rather than dropped as a dead URL.
  if (!data) return notFound();
  if (data.browsable === false) permanentRedirect('/profiles');
  const agent = data as PublicAgent;
  ...
  try {
    ...
    // "You might like" — pull a pool of recently-active browsable agents,
  ...
  const canonicalPath = `/profiles/${agent.slug || agent.id}`;
  const jsonLd = {
      ...
      {suggestions.length > 0 && (
        <section className="pt-4 border-t border-gray-100">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
            {suggestions.map(({ agent: candidate, score }) => (
              <ProfileCard
                key={candidate.id}
                agent={candidate}
                showCompatibility
